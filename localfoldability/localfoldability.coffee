...
lf.crimpable_single_vertex = (v, fold) ->
	...
	for j in [0..len-1]
		...
		k = (j+1)%%len
		i = (j-1)%%len
		prev_angle = new lf.svAngle(FOLD.geom.interiorAngle(vertex_coords[adjvertices[j]],vertex_coords[v],vertex_coords[adjvertices[i]]),[v,i],[v,j])
		curr_angle = new lf.svAngle(FOLD.geom.interiorAngle(vertex_coords[adjvertices[k]],vertex_coords[v],vertex_coords[adjvertices[j]]),[v,j],[v,k])
		next_angle = new lf.svAngle(FOLD.geom.interiorAngle(vertex_coords[adjvertices[m]],vertex_coords[v],vertex_coords[adjvertices[k]]),[v,k],[v,m])
